                 ...
                 {stats.shortestWait.map((c, i) => (
                   <div key={c.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center font-bold text-xs text-indigo-600 shadow-sm">
                          #{i + 1}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-slate-800">{c.name || `Thợ ${c.number}`}</p>
...
function ChairItem({ chair, settings, onUpdateWaiting, onForceStatus, canControl, barberName }: { 
  ...
  return (
    ...
    >
      ...
      <p className={`text-[15px] font-black uppercase mb-1 tracking-wider text-indigo-600 bg-indigo-50/50 px-3 py-1 rounded-full truncate max-w-[95%] group-hover:bg-white transition-colors border border-indigo-100 shadow-sm`}>
        {chair.name ? chair.name.toUpperCase() : `GHẾ ${chair.number < 10 ? `0${chair.number}` : chair.number}`}
      </p>
      ...
      <p className={`text-[10px] font-black mt-2 tracking-widest uppercase ${subColor}`}>
